...
fn create_dispatcher_struct(
    ...
    let ts = quote! {
        pub struct #dispatcher_name (pub std::sync::Arc<dyn #struct_name >);

        impl From<Arc<dyn #struct_name >> for #dispatcher_name {
            fn from(server: Arc<dyn #struct_name >) -> Self {
                Self(server)
            }
        }
...
pub fn service_impl(attr: TokenStream, mut item: TokenStream) -> TokenStream {
    ...
    let service_impl = quote! {
        impl #impl_name {
            ...
            pub fn create_server_request_handler(server : Arc<#impl_name>) -> Arc<dyn ServerRequestHandler> {
                std::sync::Arc::new(#dispatcher_module_name :: #dispatcher_name (server))
            }
        }
    };
